...
hardnessmodel %>%
    ...
    labs(x = "Actual", 
        ...
        title = "Second-order linear model for the hardness of CPM-Magnacut under different prep conditions",
        subtitles = "with variations in austinizing temperature, temper temperature and quench rate",
...
hardnessmodel %>%
    broom::augment(newdata = crossing(aust = seq(1900, 2200, 10),
    ...
    facet_wrap(~quench) + 
    labs(x = "Austinizing temperature (in degF)",
         y= "Temper temperature (in degC)", 
         z = "Hardness range") + 
    scale_x_continuous(sec.axis = sec_axis(~ 5/9 * (. - 32), name = "Austinizing temperature (in degC)")) + 
    scale_y_continuous(sec.axis = sec_axis(~ 5/9 * (. - 32), name = "Temper temperature (in degC)"))
